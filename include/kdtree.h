...
struct dimension_extractor {
  ...
  subtract(const Point &p1, const Point &p2, std::size_t dim) {
    ...
    return dimension_extractor<Point, Dimension + 1, Count>::subtract(p1, p2,
                                                                      dim);
  }
};
// end recursion
...
typename boost::geometry::default_distance_result<Point>::type subtract(
    const Point &p1, const Point &p2, std::size_t dim) {
  return dimension_extractor<
      Point, 0, boost::geometry::dimension<Point>::type::value>::subtract(p1,
                                                                          p2,
                                                                          dim);
}
...
class kdtree {
 public:
  ...
  void knearest(const Point &query, size_t k,
                std::vector<const Data *> &result) const {
    if (!m_root || k < 1) {
      return;
    }
  ...
  const Data *nearest_iterative(const Point &query) const {
    ...
    while (!pq.empty()) {
      ...
      double d = boost::geometry::comparable_distance(
          query, *currentNode->split);  // no sqrt
